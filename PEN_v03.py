...
def grafik_darstellung(probe,proben_seite):
    ...
    ergebnisse_1 = read_excel(roh_data_path+ "/" +str(probe)+proben_seite +".xlsm", sheet_name="Ergebnisse_1",header=None).round(decimals=6)
    ergebnisse_2 = read_excel(roh_data_path+ "/" +str(probe)+proben_seite +".xlsm", sheet_name="Ergebnisse_2",header=None).round(decimals=6)
    ergebnisse_3 = read_excel(roh_data_path+ "/" +str(probe)+proben_seite +".xlsm", sheet_name="Ergebnisse_3",header=None).round(decimals=6)
    ...
    #
    ax1.plot(penetrationstiefe_mm_1[2::],kraft_kn_1[2::],marker = "s",markevery=15,linewidth=1.5,color = "k",markersize=10,label = "Messung - 1 "+f"{arbeit_1} kN/m²")
    ax1.plot(penetrationstiefe_mm_2[2::],kraft_kn_2[2::],marker = "x",markevery=15,linewidth=1.5,color = "r",markersize=10,label = "Messung - 2 "+f"{arbeit_2} kN/m²")
    ax1.plot(penetrationstiefe_mm_3[2::],kraft_kn_3[2::],marker = "d",markevery=15,linewidth=1.5,color = "b",markersize=10,label = "Messung - 3 "+f"{arbeit_3} kN/m²")
    ...
    ax1.annotate("Spitze vollständig im Probenmaterial ",
                 xy=(30,0.82*(max_y*3)), xycoords='data',
                 xytext=(0, 0), textcoords='offset points', ha="left", fontsize=13, color="black", fontname="Arial")
    plt.rc('text', usetex=True)
    ax1.annotate(r"$w_{50}$ = $\frac{1}{A[mm^2] * s_{50} [mm]}$" + r'$\int\limits_{s = 0 [mm]}^{s_{50} = 50 [mm]}\!\! F_{c} [kN] ds (1)$',
                 xy=(30,0.9*(max_y*3)), xycoords='data',
                 xytext=(0, 0), textcoords='offset points', ha="left", fontsize=20, color="black", fontname="serif")

    plt.figtext(0.5, 0.05, r"1) Der $w_{50}$-Wert bezeichnet die für die Penetration erforderliche Arbeit (Kraft [F] x Weg [L]) bezogen auf das durch den Penetrator verdrängte Volumen [L³].", wrap=True, horizontalalignment='center', fontsize=12)
...
def haupt_funktion2(versuch_list,layout,block_ort,block_ort_pen):
                ...
                if ((versuch_list.to_numpy()[j][block_ort_pen+1] != "-" )  and (versuch_list.to_numpy()[j][block_ort_pen+2] != "-"))  ==  True   :
                    ...
                    for i in range(1,3) :
                            ...
                            if i == 1:
                                ...
                                grafik_darstellung(versuch_list.to_numpy()[j][block_ort_pen-11],i)
                    ...
                    position = probe_position(versuch_list, j, block_ort)
                    grafik_darstellung(versuch_list.to_numpy()[j][block_ort_pen - 11], 1)
                    ...
                    position = probe_position(versuch_list, j, block_ort)
                    grafik_darstellung(versuch_list.to_numpy()[j][block_ort_pen - 11], 2)
                    for i in range(1,3):
...
versuch_list = read_excel("Versuche_2022_az0100_42-43_H37-38_v04.xlsx", sheet_name="Uebersicht",header = None).replace(
        nan, "nan")
...
layout = PdfFileReader("Layout.pdf", "rb")
block_ort = 0
if block_ort == 0:
    block_ort_pen = 12
elif block_ort == 1:
    block_ort_pen = 32
haupt_funktion2(versuch_list,layout,block_ort,block_ort_pen)
